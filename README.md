...
一个监视 Bilibili 会员购市集商品价格的桌面小工具（PyQt5）。从文件导入商品链接，依次查询每个商品的现价、近期均价和最近成交记录，以表格展示。
...
- **表格展示**：
  ...
  | 缩略图 | 商品名 | 现价 | 近30天均价 | 成交① | 成交② | 成交③ | 链接 |
  | --- | --- | --- | --- | --- | --- | --- | --- |
  | img | 名称 | ¥44 | ¥205 | ¥205 · 8天前 | … | … | 打开 |
...
https://mall.bilibili.com/neul-next/index.html?page=magic-market_detail&noTitleBar=1&clusterId=10000008780
10000008780
```

程序启动时自动读取该文件并轮询一遍；之后可在界面里点"导入链接…"换文件、点"刷新"重新查询。
...
- 本工具只做价格监视，不涉及下单。
- 数据来自无文档的私有接口，本工具轮询间隔 ≥ 2 秒，请勿修改为高频轮询。
- 开发计划、待办与待确认事项见 [dev_notes/](dev_notes/)。
