...
程序启动时自动读取该文件，把清单与本地缓存合并后显示（已有缓存的商品立即显示名称和缩略图，不会自动联网）。

### 界面按钮

- **刷新商品列表**：重新读取 `watchlist.txt`，清单里新增的商品会补成新行（只填 clusterID）；原先只作为"缓存记录"显示、现在进了清单的商品会并入监视范围，之后可以抓价格。
- **抓取价格**：逐个抓取清单内商品的价格；新商品连名称、缩略图一起抓，已缓存的商品复用本地信息只刷新价格。抓取间隔 ≥ 2 秒，过程中显示进度。
